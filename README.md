...
Invoke using `inventory-ocr <input folder>` with input folder specifying a path to the inventory cards to be extracted. Results will be saved to `output/` relative to the current working directory.
...
Regions (in relative coordinates) and field names are stored in `inventory_ocr/config/regions.yaml` and can be adapted according to the use case.
